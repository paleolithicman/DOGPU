...
FgpuTargetLowering::FgpuTargetLowering(const FgpuTargetMachine &TM, const FgpuSubtarget &STI)
  ...
  //setOperationAction(ISD::LOAD, MVT::v128i8, Custom);
  //setOperationAction(ISD::STORE, MVT::i32,    Custom);
  //setOperationAction(ISD::STORE, MVT::f32,    Custom);
  //setOperationAction(ISD::STORE, MVT::v128i8, Custom);
  ...
  setOperationAction(ISD::EXTRACT_VECTOR_ELT, MVT::v128i8, Custom);
...
SDValue FgpuTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  DEBUG(dbgs() << "dsheffie: LowerOperation entered with opcode " << Op.getOpcode() << "\n");
  ...
  switch (Op.getOpcode())
    ...
    case ISD::EXTRACT_VECTOR_ELT:
      return LowerExtractVector(Op, DAG);
    default:
      assert(0);
    }

  DEBUG(dbgs() << "dsheffie : fall-thru in " << __PRETTY_FUNCTION__ << "\n");
  return SDValue();
}

SDValue FgpuTargetLowering::LowerExtractVector(SDValue Op, SelectionDAG &DAG) const {
  DEBUG(dbgs() << "dsheffie, entered : " << __PRETTY_FUNCTION__ << "\n");
  exit(-1);
  return SDValue();
}


SDValue FgpuTargetLowering::LowerBuildVector(SDValue Op, SelectionDAG &DAG) const {
  DEBUG(dbgs() << "dsheffie, entered : " << __PRETTY_FUNCTION__ << "\n");
  exit(-1);
  return SDValue();
}
...
SDValue FgpuTargetLowering::lowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  DEBUG(dbgs() << "trying to lower SETCC?\n");
...
SDValue FgpuTargetLowering::lowerStore(SDValue Op, SelectionDAG &DAG) const{
  DEBUG(dbgs() << "ahsu: lowerStore entered " << "\n");
  StoreSDNode *N = cast<StoreSDNode>(Op);
  EVT MemVT = N->getMemoryVT();
  //if (MemVT == MVT::i32 || MemVT == MVT::f32 || MemVT == MVT::v128i8) {
  //    DEBUG(dbgs() << "ahsu: getAddressSpace = " << N->getAddressSpace() << "\n");
  //    return Op;
  //}
...
SDValue FgpuTargetLowering::lowerLoad(SDValue Op, SelectionDAG &DAG) const
{
  ...
  //        // ahsu: this node is incorrect... i think?
  //        //Op = DAG.getNode(FgpuISD::SLW, DL, MVT::i32, Chain, addr);

  //        //dsheffie: maybe more correct?
  //        SmallVector <SDValue, 8> Ops;
  //        for (unsigned i = 0, e = LD->getNumOperands(); i != e; ++i) {
  //          Ops.push_back(LD->getOperand(i));
  //        }
  //        auto N = DAG.MorphNodeTo(LD, FgpuISD::SLW, LD->getVTList(), Ops);
  //        SDValue V(N, 0);
  //        return V;
  //    }
  //    return Op;
...
SDValue FgpuTargetLowering::LowerFormalArguments(SDValue Chain, 
  ...
  bool isKernel = FGPU::isKernelFunction(*F);
...
FgpuTargetLowering::ConstraintType FgpuTargetLowering::getConstraintType(StringRef Constraint) const {
  ...
  if (Constraint.size() == 1) {
    DEBUG(dbgs() << "soubhi: ConstraintType is " << Constraint << "\n");
    switch (Constraint[0]) {
      default :
	break;
      case 'f':
  ...
  DEBUG(dbgs() << "dbsheffi: fall-thru for " << Constraint << " with size " << Constraint.size() << "\n");
...
FgpuTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
	...
	case MVT::f32:
	case MVT::f64:
	  return std::make_pair(0U, &Fgpu::FloatRegsRegClass);
	default:
	  assert(false);
	...
	default:
	  DEBUG(dbgs() << "dsheffie : need to handle SimpleTy " << VT.SimpleTy << "\n");
	  assert(false);
	  // This will generate an error message
	  return std::make_pair(0u, static_cast<const TargetRegisterClass*>(0));
	}
...
void FgpuTargetLowering::LowerAsmOperandForConstraint(SDValue Op,
  ...
  SDLoc DL(Op);
  SDValue Result;
  DEBUG(dbgs() << "soubhi: LowerAsmOperandForConstraint with "<< Constraint << " entered\n");
  // Only support length 1 constraints for now.
  if (Constraint.length() > 1) {
    DEBUG(dbgs() << "dbsheffi : constraint length " << Constraint.length() << " too long, abort!\n");
    return;
  }
...
bool FGPU::isKernelFunction(const Function &F) {
  // F.print(dbgs());
  // F.viewCFG();
  // F.viewCFGOnly();
