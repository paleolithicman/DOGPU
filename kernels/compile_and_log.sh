...
#   2. If pure assembly source, use the option -s

# E.g. ./compile_and_log.sh bitonic_hard_float.cl -hard-float
# E.g. ./compile_and_log.sh bitonic_hard_float.s -s
...
do
  if [ $option = "-hard-float" ]; then # hard-float feature
    HARD_FLOAT=1
  elif [ $option = "-s" ]; then # OpenCL file to compile
    ASM=1
...
outFileName_ll=$outputDir$fileNameRoot".ll"
outFileName_s=$outputDir$fileNameRoot".s"
outFileName_bin=$outputDir$fileNameRoot".bin"
logAsmFileName=$logDir$fileNameRoot".s.log"
logObjFileName=$logDir$fileNameRoot".bin.log"
logObjDumpFileName=$logDir$fileNameRoot".objdump.log"
cFileName=$outputDir$fileNameRoot".c"
hFileName=$outputDir$fileNameRoot".h"
cramFileName=$outputDir"cram.mif"
objdumpFileName=$logDir$fileNameRoot".objdump"
...
if [ $ASM -ne 1 ]; then
  ...
  $BIN_DIR"clang" -std=c99 -O3 -Werror -S -emit-llvm  -target "mips-unknown-uknown" -o $outFileName_ll -x cl $fileName
  if [ $? != 0 ]; then
    echo "clang failed"
    exit $?
  fi
  $BIN_DIR"llc" $LLC_ARGS -filetype=asm -o $outFileName_s $outFileName_ll
  #$BIN_DIR"llc" $LLC_ARGS -filetype=asm -filter-view-dags=$BLOCKS_TO_DRAW -view-sched-dags -o $outFileName_s $outFileName_ll 
  ...
  $BIN_DIR"llvm-mc" -triple=fgpu -filetype=obj $fileName >$outFileName_bin
  if [ $? != 0 ]; then
