...
bool DAGTypeLegalizer::run() {
  ...
  while (!Worklist.empty()) {
	...
	DEBUG(dbgs() << "dsheffie : isTypeLegal " << isTypeLegal(ResultVT) << "\n");
	DEBUG(dbgs() << "dsheffie : " << __PRETTY_FUNCTION__ << " scalarizing vector?\n");
        ScalarizeVectorResult(N, i);
        Changed = true;
        goto NodeDone;
      case TargetLowering::TypeSplitVector:
	DEBUG(dbgs() << "dsheffie : isTypeLegal " << isTypeLegal(ResultVT) << "\n");
	DEBUG(dbgs() << "dsheffie : " << __PRETTY_FUNCTION__ << " splitting vector?\n");
